...
class TideNewsScraper(BaseScraper):
    HANDLER_NAME = "tide_news"
    ...
    def fetch_urls(self):
        ...
        for page_url in all_urls:
            try:
                html = self.http_client.fetch_js(page_url)
                ...
                for a in soup.select('a[href]'):
                    ...
                    if href and ('tidenews.com.cn' in href or href.startswith('/')):
                        full_url = href if href.startswith('http') else ('https:' + href if '.com' in href or '.cn' in href else 'https://tidenews.com.cn' + href)
    ...
    def parse_article(self, url):
        html = self.http_client.fetch_js(url)
        soup = BeautifulSoup(html, 'lxml')
